...
global $frontpageid;
	...
	$sidebar_layout = atp_generator( 'sidebaroption', $frontpageid );
	if( $pageslider !='' ){ $sliderclass ='iva-page_slider';}else{ $sliderclass ='';}
?>
<body <?php body_class(array($sidebar_layout,$sliderclass)); ?>>
<div class="iva-loader"></div>	
	...
	<div id="<?php echo (get_option( 'atp_layoutoption' )) ? get_option( 'atp_layoutoption' ) : 'stretched'; ?>">
	<div class="bodyoverlay"></div>
	<?php if( get_option( 'atp_stickybar' ) != "on" &&  is_active_sidebar( 'iva_stickycontent')) {  ?>
		<div id="trigger" class="tarrow"><i class="fa fa-arrow-circle-o-down fa-lg"></i></div>
		<div id="sticky"><?php  if ( is_active_sidebar( 'iva_stickycontent' ) ) : dynamic_sidebar('iva_stickycontent');  endif;  ?></div>
	<?php } ?>
