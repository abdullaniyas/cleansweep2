...
if (!class_exists('atpgenerator')) {
	class atpgenerator {
		...
		function atp_light_logo()
		{
			$atp_logo = get_option('atp_logo'); 
			if($atp_logo == 'logo'){ ?>
				<a href="<?php echo home_url(); ?>" title="<?php bloginfo('name'); ?>">
					<img src="<?php echo esc_url(get_option('atp_header_light_logo')); ?>" alt="<?php bloginfo('name'); ?>" />
				</a>
			<?php 
			}else { ?>
				<h1 id="site-title"><span><a href="<?php echo esc_url( home_url( '/' ) ); ?>" title="<?php echo esc_attr( get_bloginfo( 'name', 'display' ) ); ?>" rel="home"><?php bloginfo( 'name' ); ?>	</a></span></h1>
		...
		function atp_dark_logo()
		{
			$atp_logo = get_option('atp_logo'); 
			if($atp_logo == 'logo'){ ?>
				<a href="<?php echo home_url(); ?>" title="<?php bloginfo('name'); ?>">
					<img src="<?php echo esc_url(get_option('atp_header_dark_logo')); ?>" alt="<?php bloginfo('name'); ?>" />
				</a>
			<?php 
			}else { ?>
				<h1 id="site-title"><span><a href="<?php echo esc_url( home_url( '/' ) ); ?>" title="<?php echo esc_attr( get_bloginfo( 'name', 'display' ) ); ?>" rel="home"><?php bloginfo( 'name' ); ?>	</a></span></h1>
		...
		// Get sidebar class and adds sub class to pagemid block layout
			...
			switch($sidebaroption){
				...
				default:
						$sidebaroption= $iva_layout_option;
			}
			...
			if ( is_404() || is_singular('booking') || is_singular('offers') ) {
				$sidebaroption = 'fullwidth'; 
			}
			...
			if( get_option( 'atp_teaser' ) != 'disable' ) {
				...
				if ( $sub_option != 'disable' ) {
					...
					if( isset( $headerstyle ) && $headerstyle == 'headerstyle4'){
						if( $sh_textcolor != ''){ $sub_header_txtcolor = $sh_textcolor;}
