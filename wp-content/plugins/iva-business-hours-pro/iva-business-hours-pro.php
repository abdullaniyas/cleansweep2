 ...
 */ 
	...
	function iva_bhinstall(){
		...
		$iva_bh_columns = array('closed_bg_color','grouping_enable','toggle_enable','open_image','close_image','current_day_color','open_bg_color','opentext');
	...
	function iva_bh_page(){
		...
		if ( $results = $wpdb->get_results( $sql )) {
			...
			foreach ( $results as $value ) {
				...
				$output .= '<td>'.stripslashes( $iva_bh_shortcode ).'</td>';
		...
		echo '<li>' . __('Add shortcodes in your posts e.g: [iva_bhrs name="Office Timings" width="300" title="true" current_day_color="#ff8800" grouping_hrs="on"]', 'iva_business_hours') . '</li>';
	...
	function iva_bh_operations() {
		...
		$iva_bh_title = $iva_bh_day_open = $iva_bh_day_close = $iva_desc_prefix_checked =  $iva_desc_enable_checked = $iva_todaydate_checked = $iva_grouping_enable_checked = $iva_toggle_enable_checked = '';
		...
		if( $iva_bh_id  !=''){
			...
			foreach ( $iva_bh_results as $iva_bh_data ) {
				...
				// if( isset( $iva_bh_data ) ){
					...
					foreach( $iva_bh_day as $key => $value ){
						...
						foreach( $value as $time){
							...
							if( ( $j != 0 ) && ( iva_bh_formatTime( $time->open ,$iva_bh_time_format) != '') && ( iva_bh_formatTime( $time->open,$iva_bh_time_format ) != '' ) ){ 
								echo '<tr id="iva_bh-day-'.$key.'" class="iva_bh-day-'.$key.'" data-count="'.$iva_row_count.'">';
								echo '<td>&nbsp;</td>';
								echo '<td><span class="clockicon"><i class="aicon_clock"></i></span><input type="text" name="iva_bh['.$key.']['.$j.'][open]" value="'.iva_bh_formatTime( $time->open,$iva_bh_time_format ).'" style="width:90px;" class="iva_bh_timepicker iva_bh_input" id="iva_bh['.$key.']['.$j.'][open]"></td>';
								echo '<td><input type="text" name="iva_bh['.$key.']['.$j.'][starttime]" value="'.$start_time.'" class="iva_bh_starttime" id="iva_bh['.$key.']['.$j.'][starttime]"></td>';
				...
				echo '<div class="bhs-title">'.__('Shortcode Alias:', 'iva_business_hours').'</div><div class="bhs-desc"><input id="" type="text" class="bhrs-edit-alias" name="iva_bh_alias" value="'.$iva_bh_alias.'"><details>'.__('Slug and the shortcode ,Do not use special characters .','iva_business_hours').'</details></div><br />';
	...
	function iva_bh_insert(){
		...
		$iva_today_date = $iva_description = $iva_closed_text = $iva_open_text = $iva_closed_bg_color = $iva_open_bg_color = $iva_current_day_color = $iva_grouping_enable ='' ;
		...
		if( !$error ){
			...
			$result = $wpdb->insert( $wpdb->iva_businesshours, 
						array(
							'title'				=> $iva_bh_title,
							'alias'				=> $iva_bh_alias,
	 ...
	 */
	  if( !function_exists('iva_bh_grouping_hours')){
		function iva_bh_grouping_hours( $openHours,$iva_todaydate_enable,$today_color ){
